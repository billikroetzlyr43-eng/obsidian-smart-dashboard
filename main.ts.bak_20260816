...
class SmartDashboardView extends ItemView {
    ...
    private async renderUsageArea(card: HTMLElement): Promise<void> {
      ...
      private async renderUsageBody(body: HTMLElement): Promise<void> {
        try {
          ...
          for (const k of Object.keys(days)) {
            ...
            const d = days[k];
            monthIn += (d.hermes && d.hermes.input || 0) + (d.dsh && d.dsh.input || 0);
            monthOut += (d.hermes && d.hermes.output || 0) + (d.dsh && d.dsh.output || 0);
          }
          ...
          // 今日
          const t = days[today] || {};
          const tIn = (t.hermes && t.hermes.input || 0) + (t.dsh && t.dsh.input || 0);
          const tOut = (t.hermes && t.hermes.output || 0) + (t.dsh && t.dsh.output || 0);
      ...
      private allTotal(days: any): number {
        ...
        for (const k of Object.keys(days)) {
          const d = days[k];
          total += (d.hermes && d.hermes.input || 0) + (d.dsh && d.dsh.input || 0)
                + (d.hermes && d.hermes.output || 0) + (d.dsh && d.dsh.output || 0);
        }
        return total;
      }
      ...
      private renderMonthHeatmap(container: HTMLElement, days: any, today: string, now: Date): void {
        try {
          ...
          for (let day = 1; day <= dayCount; day++) {
            ...
            const d = days[key];
            const inp = d && (d.hermes && d.hermes.input || 0) + (d.dsh && d.dsh.input || 0) || 0;
            const out = d && (d.hermes && d.hermes.output || 0) + (d.dsh && d.dsh.output || 0) || 0;
      ...
      private renderYearHeatmap(container: HTMLElement, days: any, today: string, now: Date): void {
        try {
          ...
          for (let d = 1; d <= totalDays; d++) {
            ...
            const rec = days[key];
            const inp = rec && (rec.hermes && rec.hermes.input || 0) + (rec.dsh && rec.dsh.input || 0) || 0;
            const out = rec && (rec.hermes && rec.hermes.output || 0) + (rec.dsh && rec.dsh.output || 0) || 0;
