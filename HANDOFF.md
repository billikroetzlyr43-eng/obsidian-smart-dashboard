...
## 4. 已完成工作 (Completed)
- **核心功能/交付物：**
  - [x] `collect_usage.py` 新增 `parse_workbuddy()`，遍历 `C:/Users/<user>/.workbuddy/projects/**/*.jsonl`
...
- **技术方案与架构：**
  - 数据位置：`C:/Users/<user>/.workbuddy/projects/**/*.jsonl`，token 在 `evt.providerData.usage`（**非** `evt.usage`、**非** `evt.data.usage`）
