...
- **关键代码/文件路径：**
  - `__PLUGIN_DIR__/collect_subscriptions.py` — `DEEPSEEK_BALANCE_URL`(L39-40) / `fetch_deepseek()` / main() DeepSeek 分支
  - `__PLUGIN_DIR__/main.ts` — `SUBSCRIPTION_TEMPLATES.deepseek` / `renderSubscriptionsBody` 余额型分支（`type === 'balance'`）
  - `__PLUGIN_DIR__/styles.css` — `.sd-subscriptions-balance-value` / `.sd-subscriptions-balance-detail`
